...
Session::Session(uint64_t id) : m_id(id) {
    LOG_INFO("[Session] Created, sid={}", m_id);
}
...
void Session::set(const std::string& key, std::any value) {
    LOG_DEBUG("[Session] set called, sid={}, key={}", m_id, key);
    std::lock_guard lock(m_mutex);
    m_data[key] = std::move(value);
}

std::any Session::get(const std::string& key) const {
    LOG_DEBUG("[Session] get called, sid={}, key={}", m_id, key);
    std::lock_guard lock(m_mutex);
...
bool Session::has(const std::string& key) const {
    LOG_DEBUG("[Session] has called, sid={}, key={}", m_id, key);
    std::lock_guard lock(m_mutex);
    return m_data.count(key);
}

void Session::attach(const std::shared_ptr<Connection>& conn) {
    LOG_INFO("[Session] attach called, sid={}, conn={}", m_id, static_cast<const void*>(conn.get()));
    std::lock_guard lock(m_mutex);
    m_connections.insert(conn);
}

void Session::detach(const std::shared_ptr<Connection>& conn) {
    LOG_INFO("[Session] detach called, sid={}, conn={}", m_id, static_cast<const void*>(conn.get()));
    std::lock_guard lock(m_mutex);
    m_connections.erase(conn);
}

bool Session::empty() const {
    LOG_DEBUG("[Session] empty called, sid={}", m_id);
    std::lock_guard lock(m_mutex);
